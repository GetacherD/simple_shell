#include "main.h"
/**
* callback - callback func for CTRL+C SIG
* @signum: signal number
*/
void callback(int __attribute__((unused)) signum)
{
	write(STDOUT_FILENO, "\n$ ", 3);
}
...
int main(int __attribute__((unused)) argc, char **ar)
{
	...
	while (1)
	{
		...
		fflush(stdout);
		cmd = readline(&_read_status, &len);
		execute(&_read_status, &len, ar[0], cmd, &status, &exit_status,
			&env_last_state, &cur_state, &cur_index, &add_case, index, &count);
...
* @_read_status: last read status
* @len: number of chars read
*/

void execute(int *_read_status, ssize_t *len, char *ar, char *cmd,
	int *status, int *exit_status, int *env_last_state, int *cur_state,
		int *cur_index, int *add_case, int *index, size_t *count)
{
